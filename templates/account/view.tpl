		...
		{foreach from=$view->view_columns item=column name=columns}
			...
			{elseif $column=="t_screen_name"}
				<td>
					...
					<button type="button" class="peek" style="visibility:hidden;padding:1px;margin:0px 5px;" onclick="genericAjaxPopup('peek','c=config&a=handleSectionAction&section=twitter&action=showPeekPopup&id={$result.t_id}&view_id={$view->id}',null,false,'500');"><span class="cerb-sprite2 sprite-document-search-result" style="margin-left:2px" title="{$translate->_('views.peek')}"></span></button> 
				</td>
